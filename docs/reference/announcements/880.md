...
In earlier Camunda 8 versions the architecture looked like this

![current-state](img/current-state-refined.png)

We used ES or an OS Exporter to get data out of the distributed system (Zeebe cluster). The Zeebe cluster is where your processes are orchestrated, and then events produced are exported to a secondary data store to visualize and query.

Web applications like Tasklist and/or Operate make use of Importers and Archivers to manage process data.

Camunda 8 makes use of Elasticsearch or OpenSearch, as secondary storage.

We can see that process instances are executed by the Zeebe system. The Zeebe system exports related data to the secondary storage (Zeebe indices). Afterward, the data is imported by web applications, aggregated, and stored in their indices.

When process instances are completed, the Archivers make sure that data is moved to corresponding historic indices.

Camunda 8.8 comes with a major architecture change.

We have introduced a single application that allows us to run all necessary components. Furthermore, we are introduced
a new REST API (v2) to replace all exising REST APIs (tbd; be used by all components), and a new Camunda Exporter
that replaces the old Importer and Archivers.
...
![target](img/target-brown-field-refined-without-optimize.png)
